...
const StepAddress = ({ handleNext }) => {
  ...
  const selectedAddress = addressRadioData.filter(item => item.isSelected)[
  ...
  const [selectedAddressRadio, setSelectedAddressRadio] = useState(selectedAddress)
  ...
  const handleAddressRadioChange = prop => {
    if (typeof prop === 'string') {
      setSelectedAddressRadio(prop)
    } else {
      setSelectedAddressRadio(prop.target.value)
    }
  }
