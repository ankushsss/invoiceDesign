...
const PricingTable = ({ data }) => {
  const renderTableCell = row => {
    if (typeof row.pro === 'boolean') {
      return <Icon fontSize={20} icon={row.pro ? 'bxs:check-circle' : 'bxs:x-circle'} />
    } else {
      return <CustomChip rounded size='small' skin='light' label={row.pro} color='primary' sx={{ lineHeight: 1 }} />
    }
  }
            ...
            <TableBody>
              {data.pricingTable.rows.map((row, index) => (
                <TableRow key={index}>
                  ...
                  >
                    <Icon fontSize={20} icon={row.starter ? 'bxs:check-circle' : 'bxs:x-circle'} />
                  </TableCell>
                  <TableCell
                    align='center'
                  ...
                  >
                    <Icon fontSize={20} icon={row.enterprise ? 'bxs:check-circle' : 'bxs:x-circle'} />
                  </TableCell>
                </TableRow>
              ))}
